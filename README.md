...
**当前状态**：主线代码和教程已完成 `v0.36`（第 36 课：多 provider 与统一协议适配），阶段十继续推进。教程以默认分支的 `docs/tutorials/` 为准；运行某课时再切换该课声明的代码 tag。
...
<table width="100%">
  ...
  <tbody>
    ...
    <tr><td>v0.37–v0.39</td><td>生命周期、并行与持久交付</td><td>后续版本再加入生命周期与聚合预算、有界并行和持久化委派记录；这些能力本版不提前引入。</td></tr>
  </tbody>
</table>
