...
**当前状态**：主线代码和教程到 `v0.19`（第 19 课的单文件检查点与回滚（Checkpoint / Rollback））。教程以默认分支的 `docs/tutorials/` 为准；运行某课时再切换该课声明的代码 tag。
...
<table width="100%">
  ...
  <tbody>
    ...
    <tr><td><strong>v0.19</strong></td><td><a href="./docs/tutorials/19-checkpoint-rollback.md">单文件检查点与回滚（Checkpoint / Rollback）</a></td><td>为单文件写入保存前镜像，在冲突检测下原子恢复，并保持恢复状态可见、可验证。</td></tr>
