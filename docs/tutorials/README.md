...
本目录是默认分支持续修订的**权威教程**。课程号表示学习顺序，包版本表示发布版本，Git tag 表示代码快照；第 48 课对应进程内后台子代理。每课开头会声明自己的 tag 和相邻 diff 基线。

当前最新课程是[第 48 课：进程内后台子代理](48-background-subagents.md)，属于阶段十三。它保留同步 `delegate_task`，新增具名后台启动、状态查询、结果领取和协作取消；后台只存在于当前 CLI 进程，子会话续接留给后续版本。
...
1. **完整路线**：从[阶段一](../../README.md#stage-1)开始，依次读到[阶段十三](../../README.md#stage-13)的第 48 课。
