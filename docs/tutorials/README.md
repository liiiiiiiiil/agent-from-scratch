...
本目录是默认分支持续修订的**权威教程**。课程号表示学习顺序，包版本表示发布版本，Git tag 表示代码快照；第 41 课对应相关记忆检索。每课开头会声明自己的 tag 和相邻 diff 基线。

当前最新课程是[第 41 课：让 Agent 找回相关记忆](41-memory-retrieval.md)，属于阶段十一。它解释父 Agent 如何用有界词法检索取回少量旧资料，以及临时 Context 视图、预算和失败降级如何保持安全边界。
...
1. **完整路线**：从[阶段一](../../README.md#stage-1)开始，依次读到[阶段十一](../../README.md#stage-11)的第 41 课。
