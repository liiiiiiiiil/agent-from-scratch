...
本目录是默认分支持续修订的**权威教程**。课程号表示学习顺序，包版本表示发布版本，Git tag 表示代码快照；第 49 课对应可续接子会话。每课开头会声明自己的 tag 和相邻 diff 基线。

当前最新课程是[第 49 课：可续接子会话](49-resumable-child-session.md)，属于阶段十三。它允许父 Agent 为已领取成功结果追加完整调查合同，并将空闲子会话随 schema 4 安全点保存和恢复；活动 worker 不跨进程恢复。
...
1. **完整路线**：从[阶段一](../../README.md#stage-1)开始，依次读到[阶段十三](../../README.md#stage-13)的第 49 课。
