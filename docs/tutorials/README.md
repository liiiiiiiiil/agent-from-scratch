...
本目录是默认分支持续修订的**权威教程**。课程号表示学习顺序，包版本表示发布版本，Git tag 表示代码快照；第 42 课对应具名本地资料。每课开头会声明自己的 tag 和相邻 diff 基线。

当前最新课程是[第 42 课：具名本地资料](42-local-references.md)，属于已完成的阶段十一。它解释父 Agent 如何通过 alias、权限和逐次路径检查读取本地外部资料，同时保持 Memory、Context、session 和子代理边界。
...
1. **完整路线**：从[阶段一](../../README.md#stage-1)开始，依次读到[阶段十一](../../README.md#stage-11)的第 42 课。
