...
本目录是默认分支持续修订的**权威教程**。课程号表示学习顺序，包版本表示发布版本，Git tag 表示代码快照；第 40 课对应轻量持久 Memory。每课开头会声明自己的 tag 和相邻 diff 基线。

当前最新课程是[第 40 课：让 Agent 明确保存和修订工作区记忆](40-persistent-memory.md)，属于阶段十一。它解释父 Agent 如何显式保存工作区资料，以及权限、并发和崩溃恢复如何限制记忆写入。
...
1. **完整路线**：从[阶段一](../../README.md#stage-1)开始，依次读到[阶段十一](../../README.md#stage-11)的第 40 课。
