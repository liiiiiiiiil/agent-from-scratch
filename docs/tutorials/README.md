...
本目录是默认分支持续修订的**权威教程**。课程号表示学习顺序，包版本表示发布版本，Git tag 表示代码快照；第 35 课对应共享父子运行循环。每课开头会声明自己的 tag 和相邻 diff 基线。

当前最新课程是[第 35 课：共享父子运行循环](35-shared-agent-runtime.md)，属于阶段十。它解释为什么父 Agent 和只读 Subagent 必须共用同一个协议循环，以及如何用策略表达预算、观察、完成判定和父侧安全边界。
...
1. **完整路线**：从[阶段一](../../README.md#stage-1)开始，依次读到[阶段十](../../README.md#stage-10)的第 34 课。
