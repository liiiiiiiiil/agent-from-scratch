...
本目录是默认分支持续修订的**权威教程**。课程号表示学习顺序，包版本表示发布版本，Git tag 表示代码快照；第 39 课对应持久委派交付。每课开头会声明自己的 tag 和相邻 diff 基线。

当前最新课程是[第 39 课：持久委派交付](39-durable-delegation.md)，属于阶段十。它解释子代理结果为何要先保存为 `result_ready`，以及父进程恢复时如何按调用顺序交付原结果而不重跑子代理。
...
1. **完整路线**：从[阶段一](../../README.md#stage-1)开始，依次读到[阶段十](../../README.md#stage-10)的第 39 课。
