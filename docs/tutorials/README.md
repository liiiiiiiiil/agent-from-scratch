...
本目录是默认分支持续修订的**权威教程**。课程号表示学习顺序，包版本表示发布版本，Git tag 表示代码快照；第 32 课对应持久化工具执行边界。每课开头会声明自己的 tag 和相邻 diff 基线。

当前最新课程是[第 32 课：持久化工具执行边界](32-durable-tool-boundaries.md)，属于阶段九。它解释为什么“工具已经执行”不等于“结果已经落盘”，以及 handler 准入、单 call 提交和整轮 complete 如何约束下一次模型请求。
...
1. **完整路线**：从[阶段一](../../README.md#stage-1)开始，依次读到[阶段九](../../README.md#stage-9)的第 32 课。
