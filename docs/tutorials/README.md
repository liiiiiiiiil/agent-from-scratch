...
本目录是默认分支持续修订的**权威教程**。课程号表示学习顺序，包版本表示发布版本，Git tag 表示代码快照；第 26 课对应后台进程启动与任务边界。每课开头会声明自己的 tag 和相邻 diff 基线。

当前最新课程是[第 26 课：后台进程启动与任务边界](26-background-process-boundaries.md)，属于阶段八。它解释长命令的管道阻塞、任务专属进程归属、退出后的验证隔离，以及 `/new`、`/reset` 和 CLI 退出时的有界清理。
...
1. **完整路线**：从[阶段一](../../README.md#stage-1)开始，依次读到[阶段八](../../README.md#stage-8)的第 26 课。
