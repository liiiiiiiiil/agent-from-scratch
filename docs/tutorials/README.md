...
本目录是默认分支持续修订的**权威教程**。课程号表示学习顺序，包版本表示发布版本，Git tag 表示代码快照；第 31 课对应从完整安全点恢复会话。每课开头会声明自己的 tag 和相邻 diff 基线。

当前最新课程是[第 31 课：从完整安全点恢复会话](31-safe-resume.md)，属于阶段九。它解释为什么读取 session 不等于恢复运行时，以及 `--resume` 如何检查工作区、重建新运行时并让旧验证失效。
...
1. **完整路线**：从[阶段一](../../README.md#stage-1)开始，依次读到[阶段九](../../README.md#stage-9)的第 31 课。
