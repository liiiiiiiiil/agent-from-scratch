...
本目录是默认分支持续修订的**权威教程**。课程号表示学习顺序，包版本表示发布版本，Git tag 表示代码快照；第 37 课对应子代理生命周期与聚合预算。每课开头会声明自己的 tag 和相邻 diff 基线。

当前最新课程是[第 37 课：子代理生命周期与聚合预算](37-subagent-lifecycle-budget.md)，属于阶段十。它解释委派记录如何从创建走到父结果提交，父任务如何预留与结算聚合预算，以及同步取消怎样在请求和工具边界收束。
...
1. **完整路线**：从[阶段一](../../README.md#stage-1)开始，依次读到[阶段十](../../README.md#stage-10)的第 37 课。
