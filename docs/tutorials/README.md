...
本目录是默认分支持续修订的**权威教程**。课程号表示学习顺序，包版本表示发布版本，Git tag 表示代码快照；第 30 课对应会话持久化与安全点。每课开头会声明自己的 tag 和相邻 diff 基线。

当前最新课程是[第 30 课：会话持久化与安全点](30-session-persistence.md)，属于阶段九。它解释为什么 State 快照不等于可恢复会话，以及 `/save` 如何在完整安全点原子保存 `active`、在清理后提交 `clean`；v0.30 仍不能跨进程恢复任务。
...
1. **完整路线**：从[阶段一](../../README.md#stage-1)开始，依次读到[阶段九](../../README.md#stage-9)的第 30 课。
