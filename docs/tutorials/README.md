...
本目录是默认分支持续修订的**权威教程**。课程号表示学习顺序，包版本表示发布版本，Git tag 表示代码快照；第 33 课对应崩溃恢复与不确定副作用交接。每课开头会声明自己的 tag 和相邻 diff 基线。

当前最新课程是[第 33 课：崩溃恢复与不确定副作用交接](33-crash-recovery.md)，属于阶段九。它解释为什么“没有结果”不等于“没有执行”，以及如何在不重放调用的前提下把未执行事实、不确定事实和用户决定交接给新的 session。
...
1. **完整路线**：从[阶段一](../../README.md#stage-1)开始，依次读到[阶段九](../../README.md#stage-9)的第 33 课。
