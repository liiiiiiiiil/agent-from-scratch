...
本目录是默认分支持续修订的**权威教程**。课程号表示学习顺序，包版本表示发布版本，Git tag 表示代码快照；第 29 课对应有界管道 stdin。每课开头会声明自己的 tag 和相邻 diff 基线。

当前最新课程是[第 29 课：驱动等待输入的后台进程](29-interactive-process.md)，属于阶段八。它解释显式管道 stdin、有限文本写入和 EOF 的关系，以及为什么写入后仍须观察进程并独立验证。
...
1. **完整路线**：从[阶段一](../../README.md#stage-1)开始，依次读到[阶段八](../../README.md#stage-8)的第 29 课。
