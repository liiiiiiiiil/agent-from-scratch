...
本目录是默认分支持续修订的**权威教程**。课程号表示学习顺序，包版本表示发布版本，Git tag 表示代码快照；第 28 课对应后台进程控制。每课开头会声明自己的 tag 和相邻 diff 基线。

当前最新课程是[第 28 课：控制后台进程](28-process-control.md)，属于阶段八。它解释终止请求与确认退出的区别，以及为什么控制后仍须独立验证。
...
1. **完整路线**：从[阶段一](../../README.md#stage-1)开始，依次读到[阶段八](../../README.md#stage-8)的第 28 课。
