...
本目录是默认分支持续修订的**权威教程**。课程号表示学习顺序，包版本表示发布版本，Git tag 表示代码快照；第 45 课对应本地 Skills 的发现与按需加载。每课开头会声明自己的 tag 和相邻 diff 基线。

当前最新课程是[第 45 课：本地 Skills 发现与按需加载](45-local-skills.md)，属于阶段十二。它在第 44 课的父侧 MCP Tool 之上说明父 Agent 如何只展示 Skill 元数据、按权限读取正文，并沿用统一 Context、工具回灌和子代理隔离边界；v0.46 仍在规划。
...
1. **完整路线**：从[阶段一](../../README.md#stage-1)开始，依次读到[阶段十二](../../README.md#stage-12)的第 45 课。
