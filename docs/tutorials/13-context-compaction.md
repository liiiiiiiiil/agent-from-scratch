...
v0.13.1 解决了“上下文太长怎么办”，但同一会话中连续输入多个任务时，旧任务的
history 仍可能污染新任务。v0.13.2 增加显式任务边界：普通输入默认继续当前任务，
`/new <任务>` 清空旧任务并开始新任务，`/reset` 清空当前任务并等待下一条输入。

任务切换会原地清空 AgentState 的任务级事实（Todo、工具记录、错误、验证信息以及
后续版本中的 generation、failure、recovery 和重试预算），同时清空 history、摘要和
压缩游标；system prompt、项目指令和会话级权限授权保留。这样 state-bound 工具仍与
ContextManager 使用同一个状态对象，旧任务也不会遗留 checkpoint 或恢复预算。

当前 CLI 是同步执行的，重置只能发生在一轮工具调用全部回灌之后，不会在 assistant
tool call 与对应 tool result 之间制造孤儿消息。Structured State 每轮都会重新渲染，
并有确定性长度上限，优先保留当前 Task、目标和状态；较长的诊断历史只会被截断。
...
```bash
git diff --stat v0.12
git diff v0.12 -- src/mini_agent/context.py src/mini_agent/agent.py tests/test_context.py
```
