...
状态：已完成。实现采用 schema 3 顶层兼容、State format 2、派生 session 和逐 issue 用户交接；源 session 保持只读，恢复不 replay。claim 两阶段提交、重试一致性和故障注入矩阵均已完成验证。
...
- [x] `v0.30`–`v0.33` 各有独立、可教学的行为与测试，且保存/恢复入口在操作手册中可复现。
