...
- 不做用户显式强制约束的结构化提取；Todo 完全由模型自主判断生成，验证证据依赖模型触发的副作用，因此模型可能在未满足用户字面要求的情况下判定任务完成
- 不做失败自动重试框架、checkpoint、事务回滚或沙箱隔离
- 核心运行时不引入第三方依赖；外围用户体验可以通过可选依赖增强，但必须有标准库回退，不得改变核心协议、权限和状态语义
...
核心测试继续不依赖第三方运行时：临时目录用 `tempfile`，LLM 全部 mock，网络端到端验证只作为人工检查，不进入默认测试套件。外围可选依赖必须单独覆盖回退路径。
