...
阶段五已经建立了 Plan → Execute → Observe → Verify 的正常执行闭环；当观察或验证发现问题时，模型再 Replan 并继续执行。阶段六处理这个闭环中的异常分支：**发现执行失败，基于事实判断原因，采取受约束的恢复动作，再用新的验证证据决定继续、阻塞或失败；并能回放这条决策链，验证可靠性机制确实按约束工作。**
