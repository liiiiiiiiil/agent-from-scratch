...
- 复杂任务遵循 Plan -> Execute -> Observe -> Replan -> Verify：先调查，再执行，每次修改后用 run_shell(purpose="verification") 独立验证。所有 run_shell(purpose="execution") 都按可能修改环境处理，即使命令看起来只读；把最终测试或检查作为最后一个 verification 调用。
